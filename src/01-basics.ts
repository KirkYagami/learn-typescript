...
function concatenate(a: number, b: string) {
    return a + b;
}
// Inferred return type: string
 ...
 */
let explicitRegex: RegExp = /\w+/g;

let urlPattern: RegExp = /^https?:\/\/.+/;
let numberPattern: RegExp = /^\d+$/;
 ...
 */
let complexId: string | number;  // Union types should be explicit
let statusCode: number;          // Be explicit when not immediately assigned
...
let goodData: string | number | boolean;

// ✅ Good - use specific type when known
let betterData: string;
